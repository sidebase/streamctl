...
Before opening a [new bug report](https://github.com/sidebase/streamctl/issues/new?assignees=&labels=bug%2Cpending&projects=&template=bug-report.yaml), please:
...
- **Isolate the issue** and collect the exact commands and outputs that show it

Because streamctl works entirely on local files (the payload is read from `node_modules`), a great bug report usually needs no external reproduction repo. Instead, include:
...
To report a bug, please [open an issue](https://github.com/sidebase/streamctl/issues/new?assignees=&labels=bug%2Cpending&projects=&template=bug-report.yaml) based on our bug report template. The template will prompt you with questions that will increase our ability to scope and address the bug.
...
Both types can be created using our [enhancement issue template](https://github.com/sidebase/streamctl/issues/new?assignees=&labels=pending%2Cenhancement&projects=&template=enhancement.yml).

Before posting an enhancement, please consider the following questions:

- What problem does this enhancement fix?
- How would you recommend implementing this enhancement?
- How would this enhancement change the package?
  - Would it require a major, minor, or patch release?
  - Does it change the operational contract (exit codes, conflict kinds, the `check`/`sync` split)?

> [!IMPORTANT]
> The more impact your enhancement has on the package, the longer it may take to ship, as we need to integrate it into our release cycle.
...
2. Set up a local fork of the project:
   ```sh
   # Clone the fork
   ...
   git remote add upstream https://github.com/sidebase/streamctl
   # If you cloned a while ago, checkout the main branch and re-pull the latest changes
   git checkout main
   git pull
   ```
3. Set up the correct pnpm version, using [Corepack](https://nodejs.org/api/corepack.html), and install the dependencies
   ```sh
...
4. Create a new branch (based on the `main` branch):
   ...
   ```
5. Update the code to include your fix or enhancement
6. Add or update any tests that relate to your changes
7. Ensure that all local gates pass:
   ```sh
   pnpm lint
   pnpm typecheck
   pnpm test
   pnpm e2e:dry-run   # build + end-to-end dry-run against the synthetic payload
   ```
...
After you submit your Pull request, a member of our core team will review it. Please be patient with this process, as it can take some time depending on the team's availability.
